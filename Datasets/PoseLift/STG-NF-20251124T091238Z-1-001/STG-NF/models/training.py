...
class Trainer:
    ...
    def save_checkpoint(self, epoch, is_best=False, filename=None):
        ...
        # --- Save latest model ---
        latest_path = os.path.join(self.args.ckpt_dir, "stg_nf_latest.pth")
        torch.save(state, latest_path)

        # --- Save epoch-specific model ---
        epoch_path = os.path.join(self.args.ckpt_dir, f"stg_nf_epoch_{epoch+1}.pth")
        torch.save(state, epoch_path)

        # --- Save best model ---
        if is_best:
            best_path = os.path.join(self.args.ckpt_dir, "stg_nf_best.pth")
